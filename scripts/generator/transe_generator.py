...
def generate(dataset):
    ...
    for i in range(count):
        f.write(
            'CUDA_VISIBLE_DEVICES="1" python ../kg_train.py --method=TransE --config=../config/%s.config --order=%i\n' % (
                configName, i + 1))
    f.close()
...
def generateWeighted(dataset):
    ...
    for i in range(count):
        f.write(
            'CUDA_VISIBLE_DEVICES="1" python ../kg_train.py --method=TransE --weighted=True --config=../config/%s.config --order=%i\n' % (
                configName, i + 1))
    f.close()
