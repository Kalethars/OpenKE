...
class WTransH(Model):
    r'''
    WTransH introduces weight of triplets to improve TransH.
    '''
    ...
    def embedding_def(self):
        ...
        self.rel_embeddings = tf.get_variable(name="rel_embeddings", shape=[config.relTotal, config.hidden_size],
                                              initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        self.normal_vectors = tf.get_variable(name="normal_vectors", shape=[config.relTotal, config.hidden_size],
                                              initializer=tf.contrib.layers.xavier_initializer(uniform=False))
    ...
    def loss_def(self):
        ...
        w = tf.reciprocal(self.get_weights(in_batch=False))
    ...
    def predict_def(self):
        ...
        self.predict = tf.reduce_sum(self._calc(h_e, t_e, r_e, w_e), 1, keep_dims=True)
